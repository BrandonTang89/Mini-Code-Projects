...
static volatile int rxbufcnt = 0; /* Number of chars in buffer */
static unsigned rxbufin = 0;      /* Index of first free slot */
static unsigned rxbufout = 0;     /* Index of first occupied slot */
static volatile char rxbuf[NBUF]; /* The buffer */
...
void serial_init(void)
{
    ...
    /* Interrupt for both transmit and receive */
...
void uart_handler(void)
{
    ...
    if (UART_RXDRDY) {
        UART_RXDRDY = 0;
        rxbuf[rxbufin] = UART_RXD;
        rxbufin = (rxbufin+1) % NBUF;
        rxbufcnt++;
    }
...
{
    while (rxbufcnt <= 0) pause();
    char ch = rxbuf[rxbufout];
    rxbufout = (rxbufout+1) % NBUF;
    intr_disable();
    rxbufcnt--;
    intr_enable();
    return ch;
